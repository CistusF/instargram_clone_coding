...
export default function Direct() {
    ...
    const [chats, addChat] = useState(chatData[0].chats);
    ...
    const sendMessage = (e: KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key == "Enter" && input.value) {
            addChat([...chats, { userId: "me", chat: input.value }])
            input.value = ""
                ...
                <div className={styles.chatRoom}>
                    ...
                    <div className={styles.messages}>
                        ...
                        <div className={styles.user}>
                            <Image src="/userProfile.png" width={30} height={30} alt={"Icon"} />
                            <span className={styles.messageBox}>
                                {router.query.user}
                            </span>
                        </div>
                        {chats.map(({ userId, chat }, index) => {
                            return <div key={index} className={userId != "me" ? `${styles.user}` : ""}>
                                {userId != "me" && <Image src="/userProfile.png" width={30} height={30} alt={"Icon"} />}
                                <span className={userId != "me" ? styles.messageBox : `${styles.messageBox} ${styles.myMessage}`}>
                                    {chat}
                                </span>
                            </div>
                        })}
